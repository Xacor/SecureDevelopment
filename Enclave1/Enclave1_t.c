...
static sgx_status_t SGX_CDECL sgx_get(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_get_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_get_t* ms = SGX_CAST(ms_get_t*, pms);
	ms_get_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_get_t), ms, sizeof(ms_get_t))) {
	...
	get(_in_buf, _tmp_len, __in_ms.ms_idx);
	if (_in_buf) {
